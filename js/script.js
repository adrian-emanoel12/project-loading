...
const config={
    url: "https://54.89.40.174/routes/list"
    metodo: "GET",    
    sucesso(resposta) {
        console.log (resposta)
        const buscar_rota = JSON.parse(resposta)
        criarTabela(buscar_rota.slip);
    },
    erro(e) {
...
function criarTabela(dica) {
    ...
    vagas.innerHTML = dica.vagas;
   
    const preco_passageiro = document.createElement('td');
    tdDica.innerHTML = dica.preco_passageiro;
   
    const tr = document.createElement('tr');

    tr.appendChild(vagas);
    tr.appendChild(preco_passageiro);
